...
def procesar_linea(linea):
    ...
    # Quitar los espacios (los estoy quitando con el método strip() pero cuando le paso el argumento a la función)
    cp = linea[:9]
    dir = linea[9:29]
    tipo = int(linea[29])
    fp = int(linea[30])
...
def principal():
    ...
    while opcion != 0:
        ...
        if opcion == 1:
            ...
            if pos == -1:
                print('No existe un envío con ese cp')
            else:
            ...
            #for envio in v:      
                #cont = contar_por_tipo(v,control)
                #print(envio.direccion, procesar_direccion(envio.direccion, control))
                #c_dir_val += 1
            #print("direcciones válidas: ",c_dir_val)
...
def buscar_por_dir_y_tipo(v, dir , tipo):
    for envio in v:
        if envio.direccion.strip() == dir and envio.forma_pago == tipo:
            return envio
